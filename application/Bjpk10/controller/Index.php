...

{
    ...
    public function diceResult()
    {
        ...
        echo "recordeGuanyahe--\n";
        // 记录基本走向 table:games_jsks_basic_trend
        $this->active->recordeBasicTrend($data);
        echo "recorde--Bjpk10--BasicHistory--\n";
        //记录定位胆 table:games_jsks_basic_trend
        $this->active->recordeDinDanWei($data);
        echo "recorde--Bjpk10--DinDanWei--\n";
        //记录开奖结果 table:games_jsks_dice_history
        $this->active->recordeDiceHistory($data);
    ...
    public function gameHistory($type="",$size="",$yeshu="")
    {
        ...
        $liangMian = [
            "30"=>[
                ...
                "analyse"=>self::liangMianAnalyse($reverse['100']['ball_10']),
            ],
            "50"=>[
        ...
        $haomafenbu = [
            "30"=>[
                ...
                "analyse"=>self::haoMaFenBuAnalyse($reverse['100']['ball_10']),
            ],
            "50"=>[
    ...
    {
        $recentOpen = Db::name('games_bjpk10_dice_history')->field('issue,open_time,open_code')->limit(0, 30)->order('id desc')->select();
        foreach($recentOpen as $key => $vo){
            $recentOpen[$key]['open_code'] = explode(',',$recentOpen[$key]['open_code']);
        }
        echo json_encode($recentOpen);exit;
        return $recentOpen;
    }
    //撤销订单
