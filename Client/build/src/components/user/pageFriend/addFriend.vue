  ...
  methods: {
    ...
    searchFriend: function() {
      ...
      this.senddata(data, function(result) {
        that.searchResult = result.rearchResult;
        that.total = result.totalNum;
        that.pageNum = Math.ceil(that.total / result.listRows);
        that.page++;
        ...
        if (!result.msg) {
          ...
          if (result.fInfo) {
            ...
            that.$router.push({
              ...
              query: {
                fid: result.fInfo.id,
                nickname: result.fInfo.nickname,
                head: result.fInfo.head,
                status: 1
              }
            });
          } else {
