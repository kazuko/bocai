    ...
    <mt-tab-container swipeable="true"
      ...
      <mt-tab-container-item v-for="(value,key,index) in tabObj"
        ...
        <template v-if="key ===  '全部注单'">
          <template v-if="allOrder.length">
            <div v-for="order in allOrder"
              ...
              <div class="game-info flex">
                <div class="avatar"><img :src="order.avatar"></div>
                <span class="fz-14">{{order.gameName}}{{order.issue}}期</span>
              </div>
          ...
          <!-- loading提示 -->
          <div v-else
               class="order-empty flex justify-ct flow-col">
            <template v-if="noHistoryOrder">
              <span class="iconfont icon-empty fontc-9 empty-size"></span>
              <span class="fz-16 order-empty-tips">没有数据</span>
            </template>
            <mt-spinner v-else
                        color="#e62b00"
        ...
        <template v-if="key ===  '待开奖'">
          <template v-if="waitForOpen.length">
        ...
        <template v-if="key ===  '已中奖'">
          <template v-if="bingo.length">
            <div v-for="order in bingo"
              ...
              <div class="order-status flex justify-sb fz-14">
                <span>投注{{order.gold.split(' ')[2]}}</span>
                <span class="flex font-theme"><b class="iconfont icon-zhongjiang"></b>中奖{{order.win}}元</span>
              </div>
            </div>
          </template>
          <div v-else
               class="order-empty flex justify-ct flow-col">
            <span class="iconfont icon-empty fontc-9 empty-size"></span>
            <span class="fz-16">没有数据</span>
          </div>
        </template>
        <!-- 已撤单 -->
        <template v-if="key ===  '已撤单'">
          <template v-if="cancle.length">
        ...
        <template v-if="currentOrder">
          <div class="order-detail-info flex flow-col justify-sa">
            <div class="flex flow-col">
              <div class="game-avatar"><img :src="currentOrder.avatar"></div>
              <div class="game-name fz-16">{{currentOrder.gameName}}</div>
            </div>
          ...
          <div class="order-detail-list">
            ...
            <div class="flex order-detail-item">
              ...
              <p v-if="openNumArray"
                ...
                <template v-for="(n,index) in openNumArray">
                  <span> {{mixin_KeyToCharacter(n)}} </span>
            ...
            <div class="flex order-detail-item">
              ...
              <p class="fz-14 fontc-6">{{ splitCurrentOrderName(currentOrder.name, currentOrder.key) }}</p>
            </div>
            <div class="flex order-detail-item">
...
<script>
import switchTab from "@/components/game/6hecai/6hecai-switch";
...
export default {
  ...
  methods: {
    ...
    refundOrder(order) {
      this.senddata({
        data: { type: "destroy", game: order.key, orderNumber: order.order_number }
      });
    },
    ...
    getOddsData() {
      ...
      this.checkGameSocket(() => {
        ...
        this.senddata({ data: { type: "betHistory", game: this.gameKey } });
      });
    },
    ...
    //有连接符的玩法
    splitCurrentOrderName(value,gameKey) {
      let reg = /_|-/;
      let neeDsplit = typeof value === "string" && value.match(reg);

      if (neeDsplit) {
        let array = value.split(neeDsplit);
        let newArray = array.map( item => this.mixin_KeyToCharacter(item,gameKey) );
        return newArray.join(neeDsplit);
      } else {
        return this.mixin_KeyToCharacter(value,gameKey);
      }
