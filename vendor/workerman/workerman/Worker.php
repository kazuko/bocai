...
class Worker
{
     ...
     *　在将重新启动命令发送给子进程KILYWorksTimeReleTimes秒之后，如果这个过程还活着，那就强制杀死。
     * @var int
     */
     ...
     * 所有工人的工作PID。
     * 格式是这样的[worker_id=>[pid=>pid, pid=>pid, ..], ..]
     * @var array
     */
     ...
     * PID映射到子进程ID，格式：[worker_id=>[0=>$pid, 1=>$pid, ..], ..]。
     * @var array
     */
    ...
    public static function runAll()
    {
        ...
        static::parseCommand();
        static::daemonize();
        static::initWorkers();//判断是否是_os_linux
    ...
    protected static function forkWorkersForLinux()
    {
        ...
        // echo "[Worker->forkWorkersForLinux]\n";
        foreach (static::$_workers as $worker) {
    ...
    {
        // echo "[Worker->forkWorkersForWindows]\n";
    ...
    public static function getStartFilesForWindows() {
        // echo "[Worker->getStartFilesForWindows]\n";
        global $argv;
        $files = array();
    ...
    {
        // echo "[Worker->forkOneWorkerForWindows]\n";
    ...
    {
        // echo "[Worker->checkWorkerStatusForWindows]\n";
    ...
    {
        // echo "[Worker->forkOneWorkerForLinux]\n";
    ...
    {
        // echo "[Worker->monitorWorkersForLinux]\n";
    ...
    {
        // echo "[Worker->monitorWorkersForWindows]\n";
    ...
    {
        // echo "[Worker->checkIfChildRunning]\n";
        foreach (static::$_pidMap as $worker_id => $worker_pid_array) {
    ...
    {
        // echo "[Worker->writeStatisticsToStatusFile]\n";
        // For master process.
        if (static::$_masterPid === posix_getpid()) {
    ...
    {
        // echo "[Worker->writeConnectionsStatisticsToStatusFile]\n";
        // For master process.
        if (static::$_masterPid === posix_getpid()) {
    ...
    {
        // echo "[Worker->acceptUdpConnection]\n";
        set_error_handler(function(){});
